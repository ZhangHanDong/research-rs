...
定义 URL 到执行器的路由规则：结构化源（Reddit / GitHub / arXiv）走 `postagent`，
其它 URL 继续走 `actionbook browser`。提供三个 MVP recipe 让 orchestrator 能直接执行。

这是 `research-api-adapter` 项目的核心产出。完成后，`active-research` 在遇到 reddit、
...
- 包含三个 recipe：Reddit 帖子树、GitHub repo README 与搜索、arXiv API 查询
...
- 在 section 末尾贴一段 Topic Detection 扩展，识别 reddit / github 讨论类关键词
...
- 不修改 `packages/cli` 源码
- 不新增 Tavily / Exa / Brave / Hacker News recipe
...
场景: Reddit recipe 使用 --anonymous 抓取真实 thread
  测试:
    包: research-api-adapter
    过滤: tests/recipe_reddit_anonymous.sh
  层级: integration
  命中: postagent send, www.reddit.com
  假设 `postagent send --anonymous` 已经可用
  当 测试脚本执行 `postagent send --anonymous "https://www.reddit.com/r/rust/top.json?t=week"`
  那么 进程以退出码 "0" 结束
  并且 stdout 返回合法 JSON
  并且 JSON 根对象包含 "data" 字段
...
场景: Out-of-scope 项目明确排除
  ...
  那么 脚本退出码为 "0"
  并且 能找到明确标注：Tavily、Exa、Brave、Hacker News 不在本次 section 范围
...
- Tavily / Exa / Brave / Hacker News 搜索 API recipes
