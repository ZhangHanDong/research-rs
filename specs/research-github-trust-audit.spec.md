...
- `--html <path>` 必须写专用 GitHub trust scorecard report，报告主体必须优先展示
  `risk.score`、`risk.band`、`risk.confidence`、核心指标图表、reasons 和 evidence/gaps。
...
- 首屏 scorecard: repository、depth、stars/forks/issues/sample、`risk.score`、
  `risk.band`、`risk.confidence`。
- 指标图表: 至少展示 `risk.score`、`risk.confidence`、`max_24h_star_share`、
...
- 结论边界: 明确写出 "Not a fake/real verdict"；当 `band="unknown"` 或存在
  unavailable stats 时，解释 unknown 表示 evidence incomplete。
...
场景: --html 写出 GitHub trust scorecard
  ...
  并且 HTML 首屏包含 risk score、risk band 和 confidence
  并且 HTML 图表包含 `max_24h_star_share`、`max_daily_star_share`、`empty_bio_share`、`subscriber_star_ratio`
